...
echo "=== Tame Impala Ticket Monitor — macOS Setup ==="

EXPECTED_REPO_DIR="/Users/rymag/code/tame-impala-ticket-monitor-main"
REPO_DIR="$(cd "$(dirname "$0")/.." && pwd)"
cd "$REPO_DIR"

if [ "$REPO_DIR" != "$EXPECTED_REPO_DIR" ]; then
  echo "This setup expects the repo at:"
  echo "  $EXPECTED_REPO_DIR"
  echo "Current location:"
  echo "  $REPO_DIR"
  echo "Move or re-clone the project into /Users/rymag/code and rerun setup."
  exit 1
fi
...
MAIN_PLIST="$PLIST_DIR/com.rymag.ticket-monitor.plist"
GUARDIAN_PLIST="$PLIST_DIR/com.rymag.ticket-monitor.guardian.plist"
RELOADER_PLIST="$PLIST_DIR/com.rymag.ticket-monitor.reloader.plist"
BROWSER_HOST_PLIST="$PLIST_DIR/com.rymag.ticket-monitor.browser-host.plist"
mkdir -p "$PLIST_DIR"
...
<dict>
  <key>Label</key>
  <string>com.rymag.ticket-monitor</string>
...
<dict>
  <key>Label</key>
  <string>com.rymag.ticket-monitor.guardian</string>
  <key>ProgramArguments</key>
  <array>
    <string>${REPO_DIR}/venv/bin/python</string>
...
<dict>
  <key>Label</key>
  <string>com.rymag.ticket-monitor.reloader</string>
  <key>ProgramArguments</key>
  <array>
    <string>${REPO_DIR}/venv/bin/python</string>
...
<dict>
  <key>Label</key>
  <string>com.rymag.ticket-monitor.browser-host</string>
  <key>ProgramArguments</key>
  <array>
...
launchctl bootout "gui/$(id -u)/com.rymag.ticket-monitor.browser-host" >/dev/null 2>&1 || true
...
if [ "${BROWSER_HOST_ENABLED}" = "1" ]; then
  launchctl kickstart -k "gui/$(id -u)/com.rymag.ticket-monitor.browser-host" || true
  "${REPO_DIR}/scripts/browser_host.sh" ensure --config "${REPO_DIR}/config.yaml"
fi
launchctl kickstart -k "gui/$(id -u)/com.rymag.ticket-monitor" || true
launchctl kickstart -k "gui/$(id -u)/com.rymag.ticket-monitor.guardian" || true
launchctl kickstart -k "gui/$(id -u)/com.rymag.ticket-monitor.reloader" || true
